...
public class DirDBBean_Ac {
	
	private static DirDBBean_Ac instance = new DirDBBean_Ac();
	
	
	public static DirDBBean_Ac getInstance(){
		return instance;
		}
	...
	//영화 리스트 번호
	public int getNextAc() {
		PreparedStatement pstmt = null;
		Connection conn =null;
		ResultSet rs = null;

		try{
			String sql = "SELECT movieID FROM action ORDER BY movieID DESC";
			conn=getConnection();
	...
	//영화 리스트 출력
	public ArrayList<DirDataBean> getMovieListAc(int pageNumber,String category) {
		...
		try{
			sql = "SELECT movieID,poster,title FROM action where movieID < ? ORDER BY movieid DESC LIMIT 8";
			conn=getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1,  getNextAc()- (pageNumber -1) * 8);
			rs=pstmt.executeQuery();
	...
	//영화 상세페이지 출력
	public DirDataBean getMoviedetailAc(int movieId){
		...
		try{
			...
			pstmt = conn.prepareStatement(
					"SELECT * FROM action WHERE movieID= ?");
	...
	// 페이징 처리
	public boolean nextPageAc(int pageNumber){
		ArrayList<DirDataBean> list = new ArrayList<DirDataBean>();
		String sql = "SELECT * FROM action WHERE movieID <?";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		Connection conn = null;
...
/*               영화 관리자 페이지                   */
	// 영화 리스트
	public ArrayList<DirDataBean> adminMovieListAc(int pageNumber) {
		...
		try{
			sql = "SELECT * FROM action where movieID < ? ORDER BY movieid DESC LIMIT 15";
			conn=getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1,  getNextAc()- (pageNumber -1) * 15);
			rs=pstmt.executeQuery();
	...
	//영화 등록
	public int movieWriteAc(String poster, String title, String director,String actor, String story) {
		Connection conn = null;
		String SQL="INSERT INTO action VALUES (?, ?, ?, ?, ?, ?, ?)";
		try {
			conn=getConnection();
			PreparedStatement pstmt=conn.prepareStatement(SQL);
			pstmt.setInt(1, getNextAc());
	...
	//영화 삭제
	public int movieDeleteAc(int movieID){
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		...
		try{
			conn=getConnection();
			pstmt=conn.prepareStatement("delete from action where movieid=?");
			pstmt.setInt(1,movieID);
	...
	//수정 페이지에서 기존 내용 출력
	public DirDataBean getmovieAc(int movieID) {
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		String SQL="SELECT * FROM action WHERE movieID = ?";
	...
	//영화 내용 수정
	public int updateMovieAc(int movieID, String title, String director, String actor, String poster, String story) {
		String SQL1 = "SELECT count(*) as cnt FROM action where movieID=?";
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		int cnt=0;
		...
		try{
			if(cnt>0){
				...
				String SQL2="UPDATE action SET title=?, director=?, actor=?, poster=?, story=? WHERE movieID =?";
				conn=getConnection();
...
/*          메인페이지 출력        */	
	...
	public ArrayList<DirDataBean> getBodyDirCont(int movieID, String poster, String title, String category, String story) {
	   ...
	   try {
	      conn=getConnection();
	      pstmt=conn.prepareStatement("select * from action order by rand() limit 10");
	      rs=pstmt.executeQuery();
