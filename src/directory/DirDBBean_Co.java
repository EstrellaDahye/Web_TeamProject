...
import movie01.magazine.Bbs;

public class DirDBBean_Co {
	
	private static DirDBBean_Co instance = new DirDBBean_Co();
	
	
	public static DirDBBean_Co getInstance(){
		return instance;
		}
	...
	//영화 리스트 번호
	public int getNextCo() {
		PreparedStatement pstmt = null;
		Connection conn =null;
		ResultSet rs = null;

		try{
			String sql = "SELECT movieID FROM comedy ORDER BY movieID DESC";
			conn=getConnection();
	...
	//영화 리스트 출력
	public ArrayList<DirDataBean> getMovieListCo(int pageNumber,String category) {
		...
		try{
			sql = "SELECT movieID,poster,title FROM comedy where movieID < ? ORDER BY movieid DESC LIMIT 8";
			conn=getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1,  getNextCo()- (pageNumber -1) * 8);
			rs=pstmt.executeQuery();
	...
	//영화 상세페이지 출력
	public DirDataBean getMoviedetailCo(int movieId){
		...
		try{
			...
			pstmt = conn.prepareStatement(
					"SELECT * FROM comedy WHERE movieID= ?");
	...
	// 페이징 처리
	public boolean nextPageCo(int pageNumber){
		ArrayList<DirDataBean> list = new ArrayList<DirDataBean>();
...
/*               영화 관리자 페이지                   */
	// 영화 리스트
	public ArrayList<DirDataBean> adminMovieListCo(int pageNumber) {
		...
		try{
			sql = "SELECT * FROM comedy where movieID < ? ORDER BY movieid DESC LIMIT 15";
			conn=getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1,  getNextCo()- (pageNumber -1) * 15);
			rs=pstmt.executeQuery();
	...
	//영화 등록
	public int movieWriteCo(String poster, String title, String director,String actor, String story) {
		Connection conn = null;
		String SQL="INSERT INTO comedy VALUES (?, ?, ?, ?, ?, ?, ?)";
		try {
			conn=getConnection();
			PreparedStatement pstmt=conn.prepareStatement(SQL);
			pstmt.setInt(1, getNextCo());
	...
	//영화 삭제
	public int movieDeleteCo(int movieID){
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		...
		try{
			conn=getConnection();
			pstmt=conn.prepareStatement("delete from comedy where movieid=?");
			pstmt.setInt(1,movieID);
	...
	//수정 페이지에서 기존 내용 출력
	public DirDataBean getmovieCo(int movieID) {
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		String SQL="SELECT * FROM comedy WHERE movieID = ?";
	...
	//영화 내용 수정
	public int updateMovieCo(int movieID, String title, String director, String actor, String poster, String story) {
		String SQL1 = "SELECT count(*) as cnt FROM comedy where movieID=?";
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		...
		try{
			if(cnt>0){
				...
				String SQL2="UPDATE comedy SET title=?, director=?, actor=?, poster=?, story=? WHERE movieID =?";
				conn=getConnection();
...
/*          메인페이지 출력        */	
	...
	public ArrayList<DirDataBean> getBodyDirCont(int movieID, String poster, String title, String category, String story) {
	   ...
	   try {
	      conn=getConnection();
	      pstmt=conn.prepareStatement("select * from comedy order by rand() limit 10");
	      rs=pstmt.executeQuery();
