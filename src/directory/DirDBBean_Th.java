...
public class DirDBBean_Th {
	
	private static DirDBBean_Th instance = new DirDBBean_Th();
	
	
	public static DirDBBean_Th getInstance(){
		return instance;
		}
	...
	//영화 리스트 번호
	public int getNextTh() {
		PreparedStatement pstmt = null;
		Connection conn =null;
		ResultSet rs = null;

		try{
			String sql = "SELECT movieID FROM Thriller ORDER BY movieID DESC";
			conn=getConnection();
	...
	//영화 리스트 출력
	public ArrayList<DirDataBean> getMovieListTh(int pageNumber,String category) {
		...
		try{
			sql = "SELECT movieID,poster,title FROM Thriller where movieID < ? ORDER BY movieid DESC LIMIT 8";
			conn=getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1,  getNextTh()- (pageNumber -1) * 8);
			rs=pstmt.executeQuery();
	...
	//영화 상세페이지 출력
	public DirDataBean getMoviedetailTh(int movieId){
		...
		try{
			...
			pstmt = conn.prepareStatement(
					"SELECT * FROM THRILLER WHERE movieID= ?");
	...
	// 페이징 처리
	public boolean nextPageTh(int pageNumber){
		ArrayList<DirDataBean> list = new ArrayList<DirDataBean>();
	...
	/*               영화 관리자 페이지                   */
	// 영화 리스트
	public ArrayList<DirDataBean> adminMovieListTh(int pageNumber) {
		...
		try{
			sql = "SELECT * FROM thriller where movieID < ? ORDER BY movieid DESC LIMIT 15";
			conn=getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1,  getNextTh()- (pageNumber -1) * 15);
			rs=pstmt.executeQuery();
	...
	//영화 등록
	public int movieWriteTh(String poster, String title, String director,String actor, String story) {
		Connection conn = null;
		String SQL="INSERT INTO thriller VALUES (?, ?, ?, ?, ?, ?, ?)";
		try {
			conn=getConnection();
			PreparedStatement pstmt=conn.prepareStatement(SQL);
			pstmt.setInt(1, getNextTh());
			pstmt.setString(2, poster);
			pstmt.setString(3, title);
			pstmt.setString(4, director);
			pstmt.setString(5, actor);
			pstmt.setString(6, story);
			pstmt.setString(7,"thriller");
	...
	//영화 삭제
	public int movieDeleteTh(int movieID){
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		...
		try{
			conn=getConnection();
			pstmt=conn.prepareStatement("delete from thriller where movieid=?");
			pstmt.setInt(1,movieID);
			pstmt.executeUpdate();
			x=1;
			pstmt.close();
	...
	//수정 페이지에서 기존 내용 출력
	public DirDataBean getmovieTh(int movieID) {
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		String SQL="SELECT * FROM thriller WHERE movieID = ?";
	...
	//영화 내용 수정
	public int updateMovieTh(int movieID, String title, String director, String actor, String poster, String story) {
		String SQL1 = "SELECT count(*) as cnt FROM thriller where movieID=?";
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		int cnt = 0;
		...
		try{
			if(cnt>0){
				...
				String SQL2="UPDATE thriller SET title=?, director=?, actor=?, poster=?, story=? WHERE movieID =?";
				conn=getConnection();
...
/*          메인페이지 출력        */	
	...
	public ArrayList<DirDataBean> getBodyDirCont(int movieID, String poster, String title, String category, String story) {
	   ...
	   try {
	      conn=getConnection();
	      pstmt=conn.prepareStatement("select * from thriller order by rand() limit 10");
	      rs=pstmt.executeQuery();
