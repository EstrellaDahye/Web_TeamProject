...
<div class="moviedir_container">
	<div class="moviedir_form">
		<p class="inner-title">ANIMATION</p>
						
	<%
		DirDBBean_Ani manager = new DirDBBean_Ani();
		ArrayList<DirDataBean> list = manager.getMovieListAni(pageNumber, category);
	%>
	...
	%>
				<div class="poster_wrap">
					...
					<figcaption>
	   					...
	   					onClick="javascript:openPopup('dir_aniSellpopup.jsp?movieId=<%= list.get(i).getMovieID() %>')"/>
	  				</figcaption>
				</div>
	<%} %>
				<div id="popupLayer">					
	...
	<%	try{ 
		 	if(pageNumber != 1){ %>
				...
				onclick="location.href='dir_animation.jsp?pageNumber=<%= pageNumber-1 %>'"/>
	<%		}if(manager.nextPageAni(pageNumber +1)) { %>
