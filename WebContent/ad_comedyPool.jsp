...
<head>
	...
	<script>
		function newMovieCo(){
			location.href="ad_comedyWriteForm.jsp";
		}
		function updateMovieCo(delID){
   			location.href="ad_comedyUpdateForm.jsp?movieid="+delID;
   		}
   		function deleteMovieCo(delID){
   			var cf = confirm("이 영화를 정말로 삭제하시겠습니까? \n (삭제된 데이터는 복구 할 수 없습니다.)");
			if(cf){
   				location.href="ad_comedyPoolPro.jsp?movieid="+delID;}
   		}
   	</script>	
</head>
...
%>
	...
	try{
		...
		ArrayList<DirDataBean> list=manager.adminMovieListCo(pageNumber);
	...
	<div class="table-area">
	   	<input type="button" class="btn btn-primary" value="영화등록" onclick="newMovieCo();">
		<table>
		<tr>
	   		<th style="width:5%"> 순번
...
<% 			for(DirDataBean dtb:list){ %> 	
   		<tr>
   			...
   			<td><%= dtb.getStory() %>
   			<td>
	   		<input type="button" class="del_btn" value="수정" onclick="updateMovieCo('<%=dtb.getMovieID()%>');">
   			<input type="button" class="del_btn" value="삭제" onclick="deleteMovieCo('<%=dtb.getMovieID()%>');">
   		</tr>
   		<% } %>
   	</table>
