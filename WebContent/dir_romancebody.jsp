...
<div class="moviedir_container">
	<div class="moviedir_form">
		<p class="inner-title">ROMANCE</p>
						
	<%
		DirDBBean_Ro manager = new DirDBBean_Ro();
		ArrayList<DirDataBean> list = manager.getMovieListRo(pageNumber, category);
	%>
	...
	%>
				<div class="poster_wrap">
					...
					<figcaption>
	   					...
	   					onClick="javascript:openPopup('dir_roSellpopup.jsp?movieId=<%= list.get(i).getMovieID() %>')"/>
	  				</figcaption>
				</div>
	<%} %>
				<div id="popupLayer">					
	...
	<%	try{ 
		 	if(pageNumber != 1){ %>
				...
				onclick="location.href='dir_romance.jsp?pageNumber=<%= pageNumber-1 %>'"/>
	<%		}if(manager.nextPageRo(pageNumber +1)) { %>
