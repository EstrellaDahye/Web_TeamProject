...
<div class="moviedir_container">
	<div class="moviedir_form">
		<p class="inner-title">ACTION</p>
						
	<%
		DirDBBean_Ac manager = new DirDBBean_Ac();
		ArrayList<DirDataBean> list = manager.getMovieListAc(pageNumber, category);
	%>
	...
	%>
				<div class="poster_wrap">
					...
					<figcaption>
	   					...
	   					onClick="javascript:openPopup('dir_acSellpopup.jsp?movieId=<%= list.get(i).getMovieID() %>')"/>
	  				</figcaption>
				</div>
	<%} %>
				<div id="popupLayer">					
	...
	<%	try{ 
		 	if(pageNumber != 1){ %>
				...
				onclick="location.href='dir_action.jsp?pageNumber=<%= pageNumber-1 %>'"/>
	<%		}if(manager.nextPageAc(pageNumber +1)) { %>
