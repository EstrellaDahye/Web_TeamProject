...
<head>
	...
	<script>
		function newMovieHo(){
			location.href="ad_horrorWriteForm.jsp";
		}
		function updateMovieHo(delID){
   			location.href="ad_horrorUpdateForm.jsp?movieid="+delID;
   		}
   		function deleteMovieHo(delID){
   			var cf = confirm("이 영화를 정말로 삭제하시겠습니까? \n (삭제된 데이터는 복구 할 수 없습니다.)");
			if(cf){
   				location.href="ad_horrorPoolPro.jsp?movieid="+delID;}
   		}
   	</script>	
</head>
...
%>
	...
	try{
		...
		ArrayList<DirDataBean> list=manager.adminMovieListHo(pageNumber);
	...
	<div class="table-area">
	   	<input type="button" class="btn btn-primary" value="영화등록" onclick="newMovieHo();">
		<table>
		<tr>
	   		<th style="width:5%"> 순번
...
<% 			for(DirDataBean dtb:list){ %> 	
   		<tr>
   			...
   			<td><%= dtb.getStory() %>
   			<td>
	   		<input type="button" class="del_btn" value="수정" onclick="updateMovieHo('<%=dtb.getMovieID()%>');">
   			<input type="button" class="del_btn" value="삭제" onclick="deleteMovieHo('<%=dtb.getMovieID()%>');">
   		</tr>
   		<% } %>
   	</table>
