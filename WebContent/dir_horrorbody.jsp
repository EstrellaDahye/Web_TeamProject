...
<div class="moviedir_container">
	<div class="moviedir_form">
		<p class="inner-title">HORROR</p>
						
	<%
		DirDBBean_Ho manager = new DirDBBean_Ho();
		ArrayList<DirDataBean> list = manager.getMovieListHo(pageNumber, category);
	%>
	...
	%>
				<div class="poster_wrap">
					...
					<figcaption>
	   					...
	   					onClick="javascript:openPopup('dir_hoSellpopup.jsp?movieId=<%= list.get(i).getMovieID() %>')"/>
	  				</figcaption>
				</div>
	<%} %>
				<div id="popupLayer">					
	...
	<%	try{ 
		 	if(pageNumber != 1){ %>
				...
				onclick="location.href='dir_horror.jsp?pageNumber=<%= pageNumber-1 %>'"/>
	<%		}if(manager.nextPageHo(pageNumber +1)) { %>
