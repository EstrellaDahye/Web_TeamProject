...
<div class="moviedir_container">
	<div class="moviedir_form">
		<p class="inner-title">COMEDY</p>
						
	<%
		DirDBBean_Co manager = new DirDBBean_Co();
		ArrayList<DirDataBean> list = manager.getMovieListCo(pageNumber, category);
	%>
	...
	%>
				<div class="poster_wrap">
					...
					<figcaption>
	   					...
	   					onClick="javascript:openPopup('dir_coSellpopup.jsp?movieId=<%= list.get(i).getMovieID() %>')"/>
	  				</figcaption>
				</div>
	<%} %>
				<div id="popupLayer">					
	...
	<%	try{ 
		 	if(pageNumber != 1){ %>
				...
				onclick="location.href='dir_comedy.jsp?pageNumber=<%= pageNumber-1 %>'"/>
	<%		}if(manager.nextPageCo(pageNumber +1)) { %>
